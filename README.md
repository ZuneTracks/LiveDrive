...
3. Inspect each outer package, its inner architecture package, and their package maps before publishing. For sideload (`Release`) packages, SDK `Windows.winmd` and SDK `*Contract.winmd` files must be absent. For `Store` packages destined for Partner Center, SDK `WinMetadata\Windows.winmd` **must be present** — the Store recompiles the uploaded MSIL with .NET Native in the cloud and cannot resolve Windows Runtime types without it. App-owned runtime-component WinMDs must remain in both cases.
   ...
   **Inspect the `.appxupload`, never the loose `.appxbundle`.** A Store packaging run writes two different bundles. `AppPackages\LiveDrive_<version>_x64_arm_bundle_Store.appxupload` holds the MSIL bundle that Partner Center actually receives, and it is the only artifact whose WinMD content governs certification. The bundle sitting beside it in `AppPackages\LiveDrive_<version>_Store\` is the natively compiled sideload copy, which is *expected* to contain no SDK `Windows.winmd` at all. Checking that one reports a missing `Windows.winmd` for a perfectly valid upload. The two are also easy to tell apart by size, because `Windows.winmd` adds roughly 5 MB per architecture. The same distinction applies to configuration: an `.appxupload` produced from `Release` rather than `Store` is stripped by design and must not be submitted.
