...
4. Verify every `PackageDependency` in each Store package's manifest is obtainable on Windows 10 Mobile. Mobile stopped receiving framework-package updates at build 15254, so its Store can only provision `Microsoft.VCLibs.140.00` up to roughly `14.0.24217.0`. A package that declares a newer minimum **installs from the Store on Mobile but terminates immediately after the splash screen**, because the dependency is never satisfied. Desktop is unaffected, and sideloading is unaffected because the sideload bundle carries its own VCLibs package under `Dependencies\ARM\`. Building against the 15063 target and the .NET Native 1.7 toolchain yields a Mobile-compatible `14.0.22929.0`; a value such as `14.0.33519.0` indicates a newer toolchain leaked in and the Store build must not be published.
5. Attach the signed packages to the GitHub release along with certificate and dependency installation directions appropriate to the signing setup.
